...
async def adapt_query(cache_data_convert, *args, **kwargs):
    ...
    # Generate embedding with performance monitoring (measure full submit→result)
    embed_t0 = time.time()
    embedding_data = await chat_cache.embedding_func(pre_embedding_data)
    embed_dt = time.time() - embed_t0
    if chat_cache.log_time_func:
        chat_cache.log_time_func("embedding", embed_dt)
    chat_cache.report.embedding(embed_dt)

    def _timed_search():
        t0 = time.time()
        result = chat_cache.data_manager.search(
            embedding_data,
        ...
        dt = time.time() - t0
        if chat_cache.log_time_func:
            chat_cache.log_time_func("vector_search", dt)
        chat_cache.report.search(dt)
        return result

    cache_data_list = await asyncio.to_thread(_timed_search)
